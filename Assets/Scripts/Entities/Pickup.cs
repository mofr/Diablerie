...
public class Pickup : Entity
{
    ...
    public static Pickup Create(Vector3 position, string flippyFile, string name, string title = null, int dir = 0)
    {
        ...
        if (!CollisionMap.Fit(position, out position, mask: CollisionLayer.Item))
        {
    ...
    private void Awake()
    {
        ...
        CollisionMap.SetBlocked(Iso.MapToIso(transform.position), CollisionLayer.Item);
        animator = GetComponent<SpriteAnimator>();
    }

    private void OnDisable()
    {
        CollisionMap.SetBlocked(Iso.MapToIso(transform.position), CollisionLayer.None);
    }
